...
1. use an existing boilderplate
2. use the [express generator](/express_generator_setup.md)

### 1. use an existing boilerplate

* [The midterm boilerplate](https://github.com/lighthouse-labs/node-skeleton)  

* For building an API, use the node-skeleton-api branch (using es6 modules).
* [The node-skeleton-api boilerplate](git@github.com: DominicTremblay/node-skeleton.git) 

### 1.1 Repo Setup

* create a `<project_name>/express-api`
* in that directory initialize a git repo with `git init`
* connect to the repo `git remote add origin git@github.com:DominicTremblay/node-skeleton.git`
* pull the node-skeleton-api branch `git pull origin node-skeleton-api`
* rename the master branch to main `git branch -M main`
* remove the remote repo `git remote -rm origin`
* connect to your own repo that you created `git remote add origin <your repo address>`
...
1.2.6 `db/seeds/03_movie_genres`
...
### 2. Create a `useEffect` hook to load the movies
